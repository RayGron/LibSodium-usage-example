...
string encrypt(string msg, string &public_key_receiver, string &secret_key_sender)
{
    ...
    int r = crypto_box_easy(enc_msg.data(), dec_msg.data(), dec_msg.size(), nonce.data(), xpkr.data(), xsks.data());
    string res;
    if (r == 0)
    {
...
string decrypt(string msg, string &secret_key_receiver, string &public_key_sender)
{
    ...
    vector<unsigned char> xskr(crypto_kx_SECRETKEYBYTES);
    crypto_sign_ed25519_sk_to_curve25519(xskr.data(), skr.data());

    vector<unsigned char> xpks(crypto_kx_PUBLICKEYBYTES);
    crypto_sign_ed25519_pk_to_curve25519(xpks.data(), pks.data());

    int r = crypto_box_open_easy(dec_msg.data(), enc_msg.data(), enc_msg.size(), nonce.data(), xpks.data(), xskr.data());
    string res;
    if (r == 0)
    {
